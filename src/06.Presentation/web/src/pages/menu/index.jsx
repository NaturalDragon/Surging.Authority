...
const permission = permissionJson.menu
const FormItem = Form.Item;


/* eslint react/no-multi-comp:0 */
@connect(({ menuModel, loading }) => ({
  menuModel,
  loading: loading.models.menuModel,
}))
class TableList extends Component {
      ...
      render: (text, record) => {
        ...
        return (<Fragment>
          ...
          <Divider type='vertical'></Divider>
          {
            ...
            <AuthorA auth={permission.remove}  title="有子菜单不能删除" style={{color:'#ddd',cursor:'n'}}>删除</AuthorA>
            :
          <Popconfirm title="确定删除吗？" onConfirm={() => this.handleRemoveClick(record)}>
            <AuthorA  auth={permission.remove}>删除</AuthorA>
            </Popconfirm>
  ...
  componentDidMount() {
    ...
    // dispatch({
    //   type:'menuModel/getAllModule',
    //   payload:{}
    // })
    // dispatch({
    //   type:'menuModel/getMenuTree',
    //   payload:{}
    // })
  }
  ...
  handleSearch = e => {
    e.preventDefault();
    const { dispatch, form, menuModel: { pagination } } = this.props;
  ...
  handleFormReset = () => {
    const { form, dispatch, menuModel: { pagination } } = this.props;
    form.resetFields();
    this.setState({
      formValues: {},
    });
  ...
  handleAdd = fields => {
    ...
    if (this.state.optMode === 'add') {
      ...
      dispatch({
        type: 'menuModel/modify',
        payload: {
          ...{ ...fields, ...{ id: this.state.currentItem.value } }
        },
        callback: () => {
          this.handleModalVisible();
          this.handleFormReset()
  ...
  render() {
    const {
      menuModel: { list, pagination, dataInfo, moduleList, menuTreeList },
      loading,
    } = this.props;
