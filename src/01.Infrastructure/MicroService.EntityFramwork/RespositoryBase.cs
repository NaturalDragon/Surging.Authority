...
namespace MicroService.EntityFramwork
{
          ...
          where TEntity : class, IEntity<TPrimaryKey>
    {
      ...
      public  async Task<IEnumerable<TResult>> EntitiesByExpressionAsync<TResult>(Expression<Func<TEntity, bool>> expression,
        ...
        public async Task<IEnumerable<TResult>> EntitiesByExpressionAsync<TResult, TKey>(Expression<Func<TEntity, bool>> expression,
        Expression<Func<TEntity, TResult>> selector, Expression<Func<TEntity, TKey>> keySelector,
           bool isAsc = true) where TResult : class
        {
            using (var _dbContext = GetDbContext())
            {
                var _dbSet = _dbContext.Set<TEntity>();
                ...
                if (isAsc)
                    iOrderQuery = _dbSet.OrderBy(keySelector);
                else
                    iOrderQuery = _dbSet.OrderByDescending(keySelector);
                var data = iOrderQuery.Where(expression).Select(selector);
                return await data.ToListAsync();
            }
        }
        public async Task<IEnumerable<TResult>> EntitiesByExpressionAsync<TResult>(Expression<Func<TEntity, bool>> expression,
       Expression<Func<TEntity, TResult>> selector, Expression<Func<TEntity, Dictionary<object, QueryOrderBy>>> orderBy) where TResult : class
        {
            using (var _dbContext = GetDbContext())
            {
                var _dbSet = _dbContext.Set<TEntity>();
                IQueryable<TEntity> queryable = null;
                queryable = _dbSet.GetIQueryableOrderBy(orderBy.GetExpressionToDic()).Where(expression);
                var data = queryable.Select(selector);
                return await data.ToListAsync();
            }
        }
        ...
        /// <param name="selector">TResult</param>
        public async Task<TResult> SingleAsync<TResult>(Expression<Func<TEntity, bool>> predicate,
             Expression<Func<TEntity, TResult>> selector) where TResult : class
        {
            var entities = await EntitiesByExpressionAsync(predicate,selector);
        ...
        public async Task<IEnumerable<TEntity>> WherePaged(PageData pageData,
       Expression<Func<TEntity, bool>> whereLambda, Expression<Func<TEntity, Dictionary<object, QueryOrderBy>>> orderBy)
        {
            using (var _dbContext = GetDbContext())
            {
                var _dbSet = _dbContext.Set<TEntity>();
                var source = _dbSet.Where(e => e.IsDelete == false);
                IQueryable<TEntity> queryable = null;
                queryable = source.GetIQueryableOrderBy(orderBy.GetExpressionToDic()).Where(whereLambda);

                pageData.Total = queryable.Count();
                return await
                    queryable.Skip((pageData.PageIndex - 1) * pageData.PageSize)
                     .Take(pageData.PageSize).ToListAsync();
            }
        }

        public async Task<IEnumerable<TResult>> WherePaged<TResult>(PageData pageData,
       Expression<Func<TEntity, bool>> whereLambda, Expression<Func<TEntity, TResult>> selector, Expression<Func<TEntity, Dictionary<object, QueryOrderBy>>> orderBy)
        {
            using (var _dbContext = GetDbContext())
            {
                var _dbSet = _dbContext.Set<TEntity>();
